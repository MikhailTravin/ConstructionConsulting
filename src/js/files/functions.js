...
export function tabs() {
	...
	if (tabs.length > 0) {
		...
		tabs.forEach((tabsBlock, index) => {
			...
			// Инициализация слайдов и нижних блоков
	...
	// Инициализация слайдов и синхронизация с нижними блоками
	function initTabSlidesAndBottomBlocks(tabsBlock) {
		...
		// Функция для активации нижнего блока по data-id
		...
		tabSlides.forEach(slide => {
			slide.addEventListener('click', () => {
				...
				// Удаляем активный класс у всех слайдов в текущем табе
				const activeTabBody = slide.closest('.tabs__body');
				if (activeTabBody) {
					activeTabBody.querySelectorAll('.tabs__slide').forEach(s => s.classList.remove('_active'));
				}

				// Добавляем активный класс текущему слайду
				slide.classList.add('_active');

				// Активируем соответствующий нижний блок
		...
		if (tabsContent.length > 0) {
			...
			tabsContent.forEach((tabsContentItem, index) => {
				if (tabsTitles[index].classList.contains('_tab-active')) {
					...
					// При переключении таба активируем первый слайд и соответствующий нижний блок
					const firstSlide = tabsContentItem.querySelector('.tabs__slide:first-child');
					if (firstSlide) {
						// Удаляем активный класс у всех слайдов в этом табе
